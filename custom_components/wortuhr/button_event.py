...
from .const import COLOR_OPTIONS, EVENT_ANIMATION_OPTIONS, DOMAIN
from .services import async_show_event, async_show_text
...
async def async_setup_entry(hass: HomeAssistant, config_entry: ConfigEntry, async_add_entities: AddEntitiesCallback) -> None:
    ...
    async_add_entities([WortuhrShowEventButton(hass, config_entry, device_info, host)])
...
class WortuhrShowEventButton(ButtonEntity):
    ...
    def __init__(self, hass: HomeAssistant, config_entry: ConfigEntry, device_info: DeviceInfo, host: str) -> None:
        ...
        self._host = host
        self._attr_unique_id = f"wortuhr_show_event_{config_entry.entry_id}"
    ...
    async def async_press(self) -> None:
        ...
        event_pre_animation_state = self.hass.states.get(event_pre_animation_entity_id) if event_pre_animation_entity_id else None
        event_post_animation_state = self.hass.states.get(event_post_animation_entity_id) if event_post_animation_entity_id else None

        text = message_state.state if message_state else ""
        color = COLOR_OPTIONS.get(color_state.state, 0) if color_state else 0
        event_pre_animation = EVENT_ANIMATION_OPTIONS.get(event_pre_animation_state.state, "") if event_pre_animation_state else ""
        event_post_animation = EVENT_ANIMATION_OPTIONS.get(event_post_animation_state.state, "") if event_post_animation_state else ""

        await async_show_event(self.hass, self._host, text, color, event_pre_animation, event_post_animation, 0)
