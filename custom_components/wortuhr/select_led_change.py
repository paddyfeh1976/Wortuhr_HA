...
from .const import DOMAIN, LED_CHANGE_OPTIONS
...
async def async_setup_entry(
    ...
    async_add_entities([WortuhrLedChangeSelect(hass, config_entry, device_info, host)])


class WortuhrLedChangeSelect(SelectEntity, RestoreEntity):
    _attr_has_entity_name = True
    _attr_name = "Led Farbwechsel"
    _attr_options = list(LED_CHANGE_OPTIONS.keys())
    ...
    def __init__(
        ...
        self._host = host
        self._attr_unique_id = f"wortuhr_led_change_select_{config_entry.entry_id}"
    ...
    async def async_select_option(self, option: str) -> None:
        if option not in LED_CHANGE_OPTIONS:
            return
